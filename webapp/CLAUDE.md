...
This project is indexed in a knowledge graph via the `code-graph` MCP server. Use these tools for **refactoring impact analysis** and **codebase exploration**.

### Available Tools

- **query_code_graph** - Natural language queries about code structure
- **get_code_snippet** - Retrieve source code by qualified name
- **index_repository** - Re-index after major changes (rarely needed)

### Indexed Clojure Elements

| Element | Example | Notes |
|---------|---------|-------|
| Namespaces/Modules | `lipas.backend.core` | ~350 namespaces indexed |
| Functions (defn) | `lipas.roles/check-privilege` | With docstrings and line numbers |
| CALLS relationships | Function → Function | Who calls what |
| IMPORTS relationships | Namespace → Namespace | Dependency graph |

### Qualified Name Format

**Always use hyphens, not underscores:**
- ✅ `lipas.backend.core/ensure-permission!`
- ❌ `lipas.backend.core/ensure_permission`

### Effective Query Patterns

**Find callers of a function:**
```
"What functions call check-privilege?"
"Find all functions that call lipas.backend.core/ensure-permission!"
```

**Find what a function calls:**
```
"What functions does lipas.roles/check-privilege call?"
```

**List functions in a namespace:**
```
"Show me functions in the lipas.roles namespace"
"Show me functions defined in lipas.backend.core module"
```

**Find namespace dependencies:**
```
"What modules import lipas.roles?"
```

### Retrieving Source Code

Use `get_code_snippet` with exact qualified names:
```
lipas.roles/check-privilege
lipas.backend.core/save-sports-site!
```
...
1. **Find all callers** - Query who calls the function you want to change
2. **Get the source** - Retrieve code snippets to understand usage patterns
3. **Check imports** - Find all namespaces that depend on the module
4. **Trace call chains** - Follow CALLS relationships for deeper impact

### Example: Analyzing `check-privilege` Impact

```
Query: "What functions call check-privilege?"
Result: 8 callers across backend and frontend

Query: "What modules import lipas.roles?"
Result: 31 namespaces depend on this module

Snippet: lipas.roles/check-privilege
Result: Full source at src/cljc/lipas/roles.cljc:200-221
```

### When to Use

- **Before refactoring**: Understand full impact of changes
- **Exploring unfamiliar code**: Find entry points and call chains
- **Understanding dependencies**: Map out module relationships
- **Code review**: Verify all callers handle API changes
