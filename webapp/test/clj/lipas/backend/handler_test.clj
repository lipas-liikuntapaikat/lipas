...
(deftest suggest-route-order-endpoint-test
  (testing "Valid request returns ordered segments"
    (let [user (tu/gen-user {:db? true :admin? true})
          sports-site (-> (tu/gen-sports-site)
                          (assoc :lipas-id 98765)
                          (assoc-in [:type :type-code] 4401) ; Outdoor route
                          (assoc :name "Testreitti")
                          (assoc :status "active")
                          (assoc :event-date (utils/timestamp))
                          (assoc-in [:location :city :city-code] "091")
                          (assoc-in [:location :geometries] {:type "FeatureCollection"
          ...
          _ (core/index! search sports-site :sync)
          request-body {:lipas-id 98765
                        :activity-type "cycling"
                        :fids ["3" "1" "2"]}
          resp (app (-> (mock/request :post "/api/actions/suggest-route-order")
                        (mock/content-type "application/json")
                        (mock/body (->json request-body))))
          body (<-json (:body resp))]
      (is (= 200 (:status resp)))
      (is (vector? (:segments body)))
      (is (= 3 (count (:segments body))))
      (is (every? #(contains? % :fid) (:segments body)))
      (is (every? #(contains? % :direction) (:segments body)))
      ;; Verify segments are in the correct order (1, 2, 3)
      (is (= ["1" "2" "3"] (map :fid (:segments body))))
      (is (#{"high" "medium" "low"} (:confidence body)))))

  (testing "Request validation"
    ;; Missing required fields
    (let [resp (app (-> (mock/request :post "/api/actions/suggest-route-order")
                        (mock/content-type "application/json")
                        (mock/body (->json {:lipas-id 98765}))))]
      (is (= 400 (:status resp))))
  ...
  (testing "Sports site not found returns 404"
    (let [resp (app (-> (mock/request :post "/api/actions/suggest-route-order")
                        (mock/content-type "application/json")
                        (mock/body (->json {:lipas-id 99999 ; Non-existent
                                            :activity-type "cycling"
                                            :fids ["1" "2" "3"]}))))
          body (<-json (:body resp))]
      (is (= 404 (:status resp)))
      (is (= "Sports site not found" (:error body)))))

  (testing "Response includes warnings when appropriate"
    (let [user (tu/gen-user {:db? true :admin? true})
          sports-site (-> (tu/gen-sports-site)
                          (assoc :lipas-id 87654)
                          (assoc-in [:type :type-code] 4401)
                          (assoc :name "Test site")
                          (assoc :status "active")
                          (assoc :event-date (utils/timestamp))
                          (assoc-in [:location :city :city-code] "091")
                          (assoc-in [:location :geometries] {:type "FeatureCollection"
          ...
          resp (app (-> (mock/request :post "/api/actions/suggest-route-order")
                        (mock/content-type "application/json")
                        (mock/body (->json {:lipas-id 87654
                                            :activity-type "cycling"
                                            :fids ["1" "2" "3" "99"]})))) ; 99 doesn't exist
          body (<-json (:body resp))]
      (is (= 200 (:status resp)))
      (is (vector? (:warnings body)))
      (is (some #(re-find #"Only 1 of 4 requested segments were found" %) (:warnings body)))))

  (testing "All valid activity types accepted"
    (let [user (tu/gen-user {:db? true :admin? true})
          sports-site (-> (tu/gen-sports-site)
                          (assoc :lipas-id 76543)
                          (assoc-in [:type :type-code] 4401)
                          (assoc :name "Multi-activity site")
                          (assoc :status "active")
                          (assoc :event-date (utils/timestamp))
                          (assoc-in [:location :city :city-code] "091")
                          (assoc-in [:location :geometries] {:type "FeatureCollection"
          ...
          _ (core/upsert-sports-site!* db user sports-site)
          _ (core/index! search sports-site :sync)]
      (doseq [activity-type ["outdoor-recreation-areas"
        ...
        (let [resp (app (-> (mock/request :post "/api/actions/suggest-route-order")
                            (mock/content-type "application/json")
                            (mock/body (->json {:lipas-id 76543
                                                :activity-type activity-type
                                                :fids ["1" "2"]}))))]
          (is (= 200 (:status resp))
              (str "Activity type " activity-type " should be accepted"))))))

  (testing "Error handling for invalid geometry"
    (let [user (tu/gen-user {:db? true :admin? true})
          invalid-site (-> (tu/gen-sports-site)
                           (assoc :lipas-id 65432)
                           (assoc-in [:type :type-code] 4401)
                           (assoc :name "Invalid geometry site")
                           (assoc :status "active")
                           (assoc :event-date (utils/timestamp))
                           (assoc-in [:location :city :city-code] "091")
                           (assoc-in [:location :geometries] {:type "FeatureCollection"
                                                              :features [{:type "Feature"
                                                                          :id "1"
                                                                          :properties {:fid "1"}
                                                                          :geometry {:type "Point" ; Wrong type
                                                                                     :coordinates [0 0]}}]}))
          _ (core/upsert-sports-site!* db user invalid-site)
          _ (core/index! search invalid-site :sync)
          resp (app (-> (mock/request :post "/api/actions/suggest-route-order")
                        (mock/content-type "application/json")
                        (mock/body (->json {:lipas-id 65432
                                            :activity-type "cycling"
                                            :fids ["1"]}))))
          body (<-json (:body resp))]
      (is (= 400 (:status resp)))
      (is (string? (:error body)))
      (is (re-find #"Invalid geometry type" (:message body))))))
...
(comment
  ...
  (t/run-test suggest-route-order-endpoint-test))
