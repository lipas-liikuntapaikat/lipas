...
(defn- clean-prod-sports-site
  "Strip fields that don't belong in the canonical sports-site document:
   - history-only metadata (:doc-status, :author) — break ES strict mapping
   - enrichment outputs (:search-meta, :name-localized) — regenerated by `enrich`"
  [site]
  (dissoc site :doc-status :author :search-meta :name-localized))

(defn repro-pull!
  "Pull sports-sites from another LIPAS instance into the local DB +
   Elasticsearch. Useful for reproducing prod-only bugs locally.

   `lipas-ids` is a coll of ids. Options follow as kwargs (interleaved or
   trailing map). Each upsert+index is wrapped in a try/catch so one bad
   revision doesn't abort the batch.

   Note: re-running appends new revisions (no conflict handling on insert).
   Delete prior local data first if you need a clean snapshot.

   Options:
     :env       :prod (default) or :test
     :history?  if true, pull all revisions; otherwise just current state
     :base-url  override the v2 base URL (history still uses legacy /api)

   Examples:
     (repro-pull! [89403])
     (repro-pull! [89403 620006] :history? true)
     (repro-pull! [89403 620006] {:history? true :env :test})"
  [lipas-ids & {:keys [env history? base-url]
                :or   {env :prod history? false}}]
  (assert-running-system)
  (let [http        (requiring-resolve 'clj-http.client/get)
        upsert!*    (requiring-resolve 'lipas.backend.core/upsert-sports-site!*)
        index!      (requiring-resolve 'lipas.backend.core/index!)
        v2-base     (or base-url
                        (case env
                          :prod "https://api.lipas.fi/v2"
                          :test "https://api.lipas-test.cc.jyu.fi/v2"))
        legacy-base (case env
                      :prod "https://lipas.fi/api"
                      :test "https://lipas-test.cc.jyu.fi/api")
        url         (if history?
                      (str legacy-base "/sports-sites/history/")
                      (str v2-base "/sports-sites/"))
        u           (get-robot-user)
        fetch       (fn [id]
                      (let [{:keys [body]} (http (str url id)
                                                 {:as :json
                                                  :socket-timeout 30000
                                                  :connection-timeout 8000})]
                        (->> (if history? body [body])
                             (map clean-prod-sports-site)
                             (sort-by :event-date))))]
    (doseq [id lipas-ids]
      (let [revs (fetch id)]
        (log/infof "Pulling lipas-id=%s revs=%s" id (count revs))
        (doseq [site revs]
          (try
            (let [saved (upsert!* (db) u site)]
              (index! (search) saved))
            (catch Exception ex
              (log/errorf ex "Failed lipas-id=%s event-date=%s"
                          id (:event-date site)))))
        (log/infof "Done lipas-id=%s" id)))))
