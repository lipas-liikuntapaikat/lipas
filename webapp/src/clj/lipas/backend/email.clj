...
(defn send-ptv-service-audit-complete-email!
  [emailer to {:keys [org-name service-count summary-approved summary-changes
                      desc-approved desc-changes ui-approved ui-changes]}]
  (let [fill (fn [s]
               (-> s
                   (str/replace "{{org-name}}" org-name)
                   ...
                   (str/replace "{{desc-approved}}" desc-approved)
                   (str/replace "{{desc-changes}}" desc-changes)
                   (str/replace "{{ui-approved}}" (or ui-approved "0"))
                   (str/replace "{{ui-changes}}" (or ui-changes "0"))))]
    (.send! emailer {:subject (-> templates :fi :ptv-service-audit-complete :subject)
                     :to      to
                     :plain   (fill (-> templates :fi :ptv-service-audit-complete :text))
                     :html    (fill (-> templates :fi :ptv-service-audit-complete :html))})))
