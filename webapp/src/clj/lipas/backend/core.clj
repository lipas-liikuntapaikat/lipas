...
(defn upsert-sports-site!
  ...
  ([db user sports-site draft?]
   (let [lipas-id  (:lipas-id sports-site)
     ...
     ;;    ordering) for a posted-but-missing lipas-id. `db` is threaded so the
     ;;    image-links gate can diff :images against the stored revision.
     (check-permissions! db user (or stored sports-site) draft?)
     (when stored
