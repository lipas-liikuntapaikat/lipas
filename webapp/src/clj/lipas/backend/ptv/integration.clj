...
(defn get-org-services
  ...
  (let [token (or token (get-token org-id))
        params {:url (str service-url "/list/organization")
                :method :get
...
(defn get-org-service-channels
  [{:keys [api-url token]
   ...
   org-id]
  (let [token (or token (get-token org-id))
        params {:url (str api-url "/v11/ServiceChannel/organization/" org-id)
                :method       :get
                :headers      {:Authorization (str "bearer " token)}}]
    (-> (http params)
        :body)))
...
(defn get-service
  [{:keys [service-url token org-id]
   ...
   service-id]
  (let [token  (or token (get-token org-id))
        params {:url (str service-url "/" service-id)
                :method :get
                :headers {:Authorization (str "bearer " token)}}]
    (-> (http params)
        :body)))
...
(defn create-service-location
  [{:keys [service-location-url token]
    :or   {service-location-url (:service-location-url test-config)}}
   service-location]
  (let [org-id (-> service-location :organization :id)
        token (or token (get-token org-id))
