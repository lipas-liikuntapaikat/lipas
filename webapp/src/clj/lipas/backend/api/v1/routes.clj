...
(defn head-handler
  "Creates a HEAD handler from a GET handler.
  HEAD responses have the same status and headers as GET, but no body."
  [get-handler]
  (fn [req]
    (let [resp (get-handler req)]
      (assoc resp :body nil))))
...
Access to the hierarchical type classification system used for categorizing sports facilities."
     ...
     ["/sports-places/:sports-place-id"
      {:parameters {:query [:map [:lang {:optional true} #'v1-schema/lang]]
       ...
       :get
       {:tags ["sport-places"]
        :handler get-sports-place-by-id
        :responses {200 {:body v1-schema/legacy-sports-place}}}
       :head
       {:tags ["sport-places"]
        :no-doc true
        :handler (head-handler get-sports-place-by-id)}}]
     ["/sports-places"
      (let [list-sports-places
        ...
        {:parameters {:query v1-schema/search-params}
         ...
         {:tags ["sport-places"]
          ...
          :responses {200 {:body [:vector v1-schema/legacy-sports-place-list-item]}}}
         :head
         {:tags ["sport-places"]
          :no-doc true
     ...
     ["/swagger.json"
      {:get
       {:no-doc true
        :swagger {:info {:title "Lipas-API v1"}}
