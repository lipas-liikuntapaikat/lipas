        ...
        [:<>
         ...
         [lui/table-v2
          {:headers headers
           ...
           :in-progress? loading?
           :action-icon "refresh"
           :action-label (tr :analysis/calculate-diversity)
         ...
         {:variant "text"
          :on-click (fn [_]
                      (==> [:lipas.ui.events/confirm
                            (tr :analysis/confirm-restore-defaults)
...
(defn grid-chart []
  (let [tr (<== [:lipas.ui.subs/translator])
        chart-data (<== [::subs/grid-chart-data])
        labels {:population (tr :analysis/population-chart)
...
(defn area-chart []
  (let [tr (<== [:lipas.ui.subs/translator])
        ...
        labels {:population (tr :analysis/total-population-chart)
                :diversity-idx (tr :analysis/diversity-index-chart)
                :pwm (tr :analysis/population-weighted-diversity-chart)
                :anonymized-count (tr :general/age-anonymized)
