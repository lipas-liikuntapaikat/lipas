...
(defn set-itrs-segment-edit-mode
  [{:keys [layers ^js lmap] :as map-ctx} {:keys [geoms]}]
  (let [tr #_(:translator @db/app-db) (constantly :fi)
        ^js layer (-> layers :overlays :edits)
        source (.getSource layer)
        ...
        hover (Select. #js {:layers #js [layer]
                            :condition events-condition/pointerMove
                            :style (fn [feature]
                                     (styles/itrs-segment-style-fn feature tr true false))})
        select (Select. #js {:layers #js [layer]
                             :style (fn [feature]
                                      (styles/itrs-segment-style-fn feature tr true true))})
    ...
    (doseq [^js f features]
      (.setStyle f (fn [f]
                     (styles/itrs-segment-style-fn f tr false false))))

    (.addFeatures source features)

    (-> map-ctx
        (assoc-in [:interactions :itrs-segment-select] select)
        (assoc-in [:interactions :itrs-segment-hover] hover))))
  ...
  ([map-ctx {:keys [lipas-id geoms geom-type sub-mode problems] :as
   ...
   (let [map-ctx (-> map-ctx
     ...
     (case sub-mode
       ...
       :itrs-segment (-> map-ctx
                         (set-itrs-segment-edit-mode mode))
       (do
