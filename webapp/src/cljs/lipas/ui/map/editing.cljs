...
(defn set-ordered-segments-edit-mode!
  [{:keys [layers ^js lmap] :as map-ctx} {:keys [geoms ordered-segments]}]
  ;; Clear any existing interactions and previous ordered segments
  (let [map-ctx (-> map-ctx
                    map-utils/clear-interactions!
                    map-utils/clear-ordered-segments!
                    map-utils/enable-marker-hover!)]

    ;; Show the ordered segments on the map

    (cond-> map-ctx
      ordered-segments (map-utils/show-ordered-segments! ordered-segments geoms))))
...
(defn update-editing-mode!
  ...
  (let [old-mode (:mode map-ctx)
    ...
    (cond
      ...
      (= :ordered-segments (:sub-mode mode))
      (-> map-ctx
          (set-editing-mode! mode))
