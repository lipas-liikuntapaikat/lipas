...
(rf/reg-event-fx ::confirm-remove-segment
                 (fn [{:keys [db]} [_ segment-ids callback]]
                   (let [tr (-> db :translator)
                         geom-type (-> db :map :mode :geom-type)
                         lipas-id (-> db :map :mode :lipas-id)
                         sports-site (or (get-in db [:sports-sites lipas-id :editing])
                                         (let [latest (get-in db [:sports-sites lipas-id :latest])]
                                           (get-in db [:sports-sites lipas-id :history latest])))
                         locale (tr)
                         affected-routes-map (when (and segment-ids sports-site)
                                               (geom-protection/check-multiple-segments-usage
                                                sports-site segment-ids))
                         base-message (tr :map/confirm-remove geom-type)
                         ;; Build enhanced message with route warnings if needed
                         message (if (seq affected-routes-map)
                                   (str base-message "\n\n"
                                        "⚠️ " (tr :warning/segment-deletion-affects-routes) "\n\n"
                                        (tr :warning/affected-routes-list) "\n"
                                        (reduce (fn [acc [segment-id routes]]
                                                  (str acc
                                                       (reduce (fn [route-str route]
                                                                 (str route-str
                                                                      "• " (get-in route [:route-name locale] "Nimetön reitti")
                                                                      " (" (name (:activity-k route)) ")\n"))
                                                               ""
                                                               routes)))
                                                ""
                                                affected-routes-map))
                                   base-message)]
                     {:dispatch
                      [:lipas.ui.events/confirm message callback]})))
...
(rf/reg-event-fx ::edit-site
                 (fn [{:keys [db]} [_ lipas-id geom-type can-publish? edit-activities? edit-floorball?]]
                   ;; Only enter editing mode if user has can-publish? permission
                   ;; For users with only activities/floorball permissions, keep map in default mode
                   (if can-publish?
                     ;; User has full editing permissions - enter editing mode
                     {:dispatch-n
                      [[:lipas.ui.sports-sites.events/edit-site lipas-id]
                       [::clear-undo-redo]
                       [::start-editing lipas-id :editing geom-type]]}

                     ;; User only has activities/floorball permissions - don't enter editing mode
                     (let [current-tab (get-in db [:map :selected-sports-site-tab])
                           tab-dispatch (cond
                                          ;; Jump to activities tab if user has activities permissions
                                          (and edit-activities? (not= 5 current-tab))
                                          [::select-sports-site-tab 5]

                                          ;; Jump to floorball tab if user has floorball permissions
                                          (and edit-floorball? (not= 3 current-tab))
                                          [::select-sports-site-tab 3]

                                          ;; Default case - no tab jump
                                          :else nil)]
                       {:dispatch-n
                        (filterv some?
                                 [[:lipas.ui.sports-sites.events/edit-site lipas-id]
...
(defn enrich-routes-with-lengths
  "Enriches all routes in the sports site data with calculated lengths based on current geometries"
  [sports-site]
  (let [geometries (get-in sports-site [:location :geometries])]
    (if (and geometries (-> sports-site :activities seq))
      (update sports-site :activities
              (fn [activities]
                (reduce-kv
                 (fn [acc activity-k activity-data]
                   (if-let [routes (:routes activity-data)]
                     (assoc acc activity-k
                            (update activity-data :routes
                                    (fn [routes]
                                      (mapv (fn [route]
                                              (let [filtered-geoms (when (:fids route)
                                            ...
                                            routes))))
                     (assoc acc activity-k activity-data)))
                 {}
                 activities)))
      sports-site)))
...
(rf/reg-event-fx ::save-new-site
                 (fn [{:keys [db]} [_ data]]
                   (let [draft? false
                         ...
                         data (-> data
                                  enrich-routes-with-lengths ; Calculate route lengths from current geometries
                                  utils/make-saveable
...
(rf/reg-event-fx ::select-sports-site-tab
                 (fn [{:keys [db]} [_ tab]]
                   (let [previous-tab (get-in db [:map :selected-sports-site-tab])
                         lipas-id (get-in db [:map :selected-sports-site :lipas-id])]
                     {:db (assoc-in db [:map :selected-sports-site-tab] tab)
                      :fx (cond-> []
             ;; When leaving REITIT tab (7)
                            (and (= 7 previous-tab) (not= 7 tab))
                            (conj [:dispatch [:lipas.ui.sports-sites.routes.events/deactivate-route-mode]])

             ;; When entering REITIT tab (7)
                            (and (= 7 tab) lipas-id)
                            (conj [:dispatch [:lipas.ui.sports-sites.routes.events/activate-route-mode lipas-id]])

             ;; When leaving AKTIVITEETIT tab (5) - clear activities mode
                            (and (= 5 previous-tab) (not= 5 tab))
                            (conj [:dispatch [:lipas.ui.sports-sites.activities.events/clear]]))})))

(rf/reg-event-db ::select-new-sports-site-tab
                 (fn [db [_ tab]]
                   (assoc-in db [:map :selected-new-sports-site-tab] tab)))

(rf/reg-event-db ::clear-selected-features
                 (fn [db _]
...
(rf/reg-event-fx ::highlight-route-features
                 (fn [{:keys [db]} [_ lipas-id fids]]
                   (let [;; Check current map mode
                         current-mode (get-in db [:map :mode])
                         current-mode-name (:name current-mode)
          ;; Get the sports site data (either from editing or display)
                         editing-data (get-in db [:sports-sites lipas-id :editing])
                         latest (get-in db [:sports-sites lipas-id :latest])
                         display-data (get-in db [:sports-sites lipas-id :history latest])
                         sports-site (or editing-data display-data)
                         geometries (get-in sports-site [:location :geometries])]

                     (cond
        ;; No geometries available - can't highlight
                       (not geometries)
                       {:db (assoc-in db [:map :mode :selected-features] #{})}

        ;; Clear highlights when fids is empty
                       (or (not fids) (empty? fids))
                       {:db (-> db
                                (assoc-in [:map :mode :selected-features] #{})
                 ;; Clear the highlight source in default mode
                                (cond-> (not= current-mode-name :editing)
                                  (update-in [:map :mode] dissoc :highlight-source)))}

        ;; We're in editing mode - just update selected features
        ;; The geoms should already be set correctly
                       (= current-mode-name :editing)
                       {:db (assoc-in db [:map :mode :selected-features] fids)}

        ;; We're not in editing mode (read-only) - set up view-only highlighting
        ;; Store the highlight source data for the map to use
                       :else
                       {:db (-> db
                 ;; Store which features to highlight
                                (assoc-in [:map :mode :selected-features] fids)
                 ;; Store the lipas-id and geometries for the highlight display
                                (assoc-in [:map :mode :highlight-source] {:lipas-id lipas-id
...
(rf/reg-event-fx ::set-segment-itrs-property
                 (fn [{:keys [db]} [_ lipas-id fid property-key value]]
                   (let [geoms (-> db
                                   (get-in [:map :mode :geoms])
                                   (update :features
                                           (fn [fs]
                                             (if (seq value)
                                               (update-feature-properties fs fid assoc property-key value)
                                               (update-feature-properties fs fid dissoc property-key)))))]
                     {:fx [[:dispatch [::update-geometries lipas-id geoms]]]})))

(rf/reg-event-fx
 ::set-ordered-segments
 (fn [{:keys [db]} [_ ordered-segments]]
   {:db (assoc-in db [:map :mode :ordered-segments] ordered-segments)
    :fx [[:dispatch [::refresh-ordered-segments-display]]]}))

(rf/reg-event-fx
 ::clear-ordered-segments
 (fn [{:keys [db]} _]
   {:db (update-in db [:map :mode] dissoc :ordered-segments)
    :fx [[:dispatch [::refresh-ordered-segments-display]]]}))

(rf/reg-event-db
 ::set-ordered-segments-edit-mode
 (fn [db [_ lipas-id segments]]
   (-> db
       (assoc-in [:map :mode :sub-mode] :ordered-segments)
       (assoc-in [:map :mode :lipas-id] lipas-id)
       (assoc-in [:map :mode :ordered-segments] segments))))

(rf/reg-event-db
 ::refresh-ordered-segments-display
 (fn [db _]
   ;; This event triggers a map update through the map component's lifecycle
   ;; The actual display update happens in the map editing functions
   db))
