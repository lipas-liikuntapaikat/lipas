...
(r/defc new-service-form [{:keys [org-id tr service data]}]
  (let [{:keys [source-id sub-category-id]} service
        ...
        service-candidate-descriptions @(rf/subscribe [::subs/service-candidate-descriptions org-id])
        {:keys [summary description]} (get service-candidate-descriptions source-id)

        city @(rf/subscribe [:lipas.ui.sports-sites.subs/city (-> data :location :city :city-code)])
        type-data @(rf/subscribe [:lipas.ui.sports-sites.subs/type-by-type-code (-> data :type :type-code)])
