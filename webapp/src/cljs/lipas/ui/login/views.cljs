...
(r/defc error-msg [{:keys [tr error]}]
...
(r/defc magic-link-form [{:keys [tr]}]
  (let [form-data @(rf/subscribe [::subs/login-form])
        ...
        link-ordered? @(rf/subscribe [::subs/magic-link-ordered?])]
...
(r/defc register-btn [{:keys [tooltip]}]
  [:> Tooltip
   {:title tooltip}
   [:> IconButton {:href "/rekisteroidy"}
    [:> Icon {} "group_add"]]])

(r/defc login-panel [{:keys [tr]}]
  (let [login-mode @(rf/subscribe [::subs/login-mode])
...
(r/defc main []
  (let [tr @(rf/subscribe [:lipas.ui.subs/translator])
        ...
        comeback-path @(rf/subscribe [::subs/comeback-path])
