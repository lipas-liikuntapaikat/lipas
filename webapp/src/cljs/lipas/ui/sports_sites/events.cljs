...
(rf/reg-event-fx ::switch-to-routes-tab
  (fn [{:keys [db]} [_ lipas-id route-id]]
    ;; Tab 7 is the REITIT tab
    {:db db
     :fx [[:dispatch [:lipas.ui.map.events/select-sports-site-tab 7]]
          ;; If a specific route is provided, select it in the routes view
          (when route-id
            [:dispatch [:lipas.ui.sports-sites.activities.events/select-route lipas-id
                        {:id route-id}]])]}))
