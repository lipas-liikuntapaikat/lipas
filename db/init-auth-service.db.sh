...
psql -h $DB_HOST -p $DB_PORT -U $DB_SUPERUSER <<EOF
     ...
     DROP DATABASE IF EXISTS $AUTH_TEST_DB_NAME;
     ...
     CREATE DATABASE $AUTH_TEST_DB_NAME WITH OWNER=$AUTH_DB_USER ENCODING='utf-8';
     GRANT ALL PRIVILEGES ON DATABASE $AUTH_TEST_DB_NAME to $AUTH_DB_USER;
EOF
...
psql -h $DB_HOST -p $DB_PORT -U $DB_SUPERUSER -d $AUTH_TEST_DB_NAME <<EOF
     CREATE EXTENSION IF NOT EXISTS citext;
     CREATE EXTENSION IF NOT EXISTS "uuid-ossp";
EOF
